...
class CityMapper:
    def __init__(self):
        ...
        plt_params_dict = {"figure.facecolor": (1.0, 1.0, 1.0, 1.0), "font.family": "serif"}
        plt.rcParams.update(plt_params_dict)

    def load_data_from_city(
        self, city_name: str, city_elements: dict = None, city_limits: bool = True
    ):
        """
    ...
    def load_data_from_address(self, address, distance=500, city_elements: dict = None):
    ...
    def _get_city_data_within_city_limits(self):
        ...
        if self.city_dict["water"] is True:
            self.water = ox.geometries_from_polygon(
                self.city_area.unary_union, tags=self.water_tags
            )
        if self.city_dict["green"] is True:
            self.green = ox.geometries_from_polygon(
                self.city_area.unary_union, tags=self.green_tags
            )
        if self.city_dict["buildings"] is True:
            self.buildings = ox.geometries_from_place(
                self.city_name, tags={"building": True, "landuse": "construction"}
            )
    ...
    def _get_city_data_within_rectangle(self):
        if self.city_dict["cycleways"] is True:
            self.cycleways = self._get_cycleways()
        if self.city_dict["roads"] is True:
            self.roads = ox.graph_from_bbox(
                self.north, self.south, self.east, self.west, network_type="drive"
            )
        if self.city_dict["water"] is True:
            self.water = ox.geometries.geometries_from_bbox(
                self.north, self.south, self.east, self.west, tags=self.water_tags
            )
        if self.city_dict["green"] is True:
            self.green = ox.geometries.geometries_from_bbox(
                self.north, self.south, self.east, self.west, tags=self.green_tags
            )
        if self.city_dict["buildings"] is True:
        ...
        if self.city_dict["railways"] is True:
            self.railways = ox.geometries_from_bbox(
                self.north, self.south, self.east, self.west, tags=self.railway_tags
            )
    ...
    def _get_city_data_from_address(self):
        if self.city_dict["cycleways"] is True:
            self.cycleways = self._get_cycleways()
        if self.city_dict["roads"] is True:
            self.roads = ox.graph_from_address(
                self.address, network_type="drive", dist=self.distance
            )
        if self.city_dict["water"] is True:
            self.water = ox.geometries_from_address(
                self.address, tags=self.water_tags, dist=self.distance
            )
        if self.city_dict["green"] is True:
            self.green = ox.geometries_from_address(
                self.address, tags=self.green_tags, dist=self.distance
            )
        if self.city_dict["buildings"] is True:
            self.buildings = ox.geometries_from_address(
                self.address, tags={"building": True, "landuse": "construction"}, dist=self.distance
            )
        if self.city_dict["railways"] is True:
            self.railways = ox.geometries_from_address(
                self.address, tags=self.railway_tags, dist=self.distance
            )
    ...
    def _get_cycleways(self):
        ...
        if self.query_type == "city":
            if self.city_limits is True:
                cycleways = ox.graph_from_place(
                    query=self.city_name, network_type="bike", simplify=False
                )
            else:
                cycleways = ox.graph_from_bbox(
                    self.north,
                    self.south,
                    self.east,
                    self.west,
        ...
        non_cycleways = [
            (u, v, k)
            for u, v, k, d in cycleways.edges(keys=True, data=True)
            if not ("cycleway" in d or d["highway"] == "cycleway" or d["highway"] == "path")
        ]
    ...
    def _update_plot_params(self, colors, edge_colors, edge_width):
        ...
        if edge_width is None:
            edge_width = self.auto_resize_elements()
        if edge_width is None:
            self.edge_width_dict.update({"roads": 0.75, "water": 1, "railways": 0.25})
        self.edge_width_dict.update(edge_width)

    def auto_resize_elements(self):
        width = self.city_area.bbox_east - self.city_area.bbox_west
        height = self.city_area.bbox_north - self.city_area.bbox_south
        size = np.max([width, height])

        inc = 0.05 - size
        if inc < -1:
            return None
        edge_width = dict(
            zip(self.edge_width_dict.keys(), [i + inc for i in self.edge_width_dict.values()])
        )
        return edge_width
    ...
    def _plot_city_area(self, ax):
        if (self.colors_dict["city_area"] is not None) & (self.city_area is not None):
            self.city_area.plot(ax=ax, facecolor=self.colors_dict["city_area"])
    ...
    def _plot_green(self, ax):
        if (self.colors_dict["green"] is not None) & (self.green is not None):
            ox.plot_footprints(
                ...
                edge_color=self.edge_colors_dict["green"],
                edge_linewidth=self.edge_width_dict["green"],
                bgcolor="white",
                show=False,
                close=False,
            )
    ...
    def _plot_water(self, ax):
        if (self.colors_dict["water"] is not None) & (self.water is not None):
            ox.plot_footprints(
                ...
                edge_color=self.edge_colors_dict["water"],
                edge_linewidth=self.edge_width_dict["water"],
                bgcolor="white",
                show=False,
                close=False,
            )
    ...
    def _plot_buildings(self, ax):
        if (self.colors_dict["buildings"] is not None) & (self.buildings is not None):
            ox.plot_footprints(
                ...
                edge_color=self.edge_colors_dict["buildings"],
                edge_linewidth=self.edge_width_dict["buildings"],
                show=False,
                close=False,
            )
    ...
    def _set_title(self, ax, title, road_cycleway_ratio_subtitle):
        ...
        if road_cycleway_ratio_subtitle is False:
            ...
            ax.text(
                s=f"Road to cycleway ratio is {rc_ratio_round}:1",
                x=0.5,
                y=1.04,
                transform=ax.transAxes,
                horizontalalignment="center",
    ...
    def savefig(self, fig, extension=["png", "pdf"], filename=None, image_dir="examples", dpi=1000):
        ...
        for ext in extension:
            full_filename = f"{image_dir}/{ext}/{filename}.{ext}"
            fig.savefig(
                full_filename, dpi=dpi, facecolor="w", transparent=False, bbox_inches="tight"
            )
